...
export function InstitusiSection() {
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      const payload = { name: form.name, type: form.type, logoUrl: form.logoUrl || undefined };
      if (editId) {
  ...
  return (
    <div>
      ...
      <Modal
        isOpen={modalOpen}
        onClose={() => { setModalOpen(false); setEditId(null); setError(""); }}
        title={editId ? "Edit Institusi" : "Tambah Institusi"}
        footer={
          <>
            <ModalCancelButton onClick={() => { setModalOpen(false); setEditId(null); setError(""); }} />
      ...
      >
        ...
        <FormField label="Logo Institusi (Opsional)">
          <DragDropUpload value={form.logoUrl} onChange={(url) => setForm(f => ({ ...f, logoUrl: url }))} label="Upload Logo Institusi" />
        </FormField>
      </Modal>
