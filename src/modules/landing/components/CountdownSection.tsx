...
function CountdownDigit({ digit }: { digit: string }) {
  return (
    <div className="relative flex h-24 w-18 items-center justify-center overflow-hidden rounded-[1.25rem] bg-[#8B5AF7] shadow-[0_8px_0_rgba(0,0,0,0.04),0_16px_18px_rgba(78,44,150,0.12)] sm:h-70 sm:w-54 sm:rounded-[1.9rem] md:h-36 md:w-30 md:rounded-[1.75rem]">
    ...
    <div className="flex flex-col items-center">
      <div className="flex flex-nowrap items-center gap-1.5 sm:gap-3">
        {value.split("").map((digit, index) => (
          <CountdownDigit key={`${label}-${index}`} digit={digit} />
        ))}
      </div>
      <p className="mt-3 text-[1rem] font-black uppercase tracking-[-0.04em] text-[#050505] sm:mt-5 sm:text-[2rem] md:text-[1.45rem]">
        {label}
      </p>
    </div>
  );
  ...
  return (
    <section className="overflow-hidden bg-[#D7FF63] py-20 text-center text-[#0A0A0A] sm:py-28">
      <div className="mx-auto flex w-[87.5%] flex-col items-center">
        <h2 className="max-w-[12ch] text-[clamp(2.4rem,4.5vw,4.9rem)] font-black italic leading-[0.95] tracking-tighter text-black sm:max-w-none sm:text-[clamp(3rem,4.5vw,4.9rem)]">
          Event Dimulai Dalam?
        </h2>
        ...
        >
          <CountdownUnit label="HARI" value={countdown.days} />
